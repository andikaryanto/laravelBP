...
class GenerateEntity extends Command
{
    ...
    public function handle()
    {
        ...
        unset($entityArray[count($entityArray) - 1]);
        unset($entityArray[0]);
    ...
    protected function createEntityFile()
    {
        ...
        foreach ($this->entityProps as $entityProp) {
            ...
            $functions .= "\tprotected function set{$function}({$returnType} $" . $property ."): {$this->className}\n" .
            "\t{\n" .
...
{$functions}
    ...
    protected function createYmlFile()
    {
        ...
        file_put_contents(app_path() . '/Entities/Mapping/' . $this->getClassName() . '.yml', $yaml,);
    }
